...
export default async function DynamicPage({ params }: CampaniaMarketingPageProps) {
  ...
  try {
    ...
    const apiUrl = `${process.env.NEXTAUTH_URL}/api/maker?p=${itemId as string}`; // Ruta a tu API Route dinámica
    ...
    if (!res.ok) {
     ...
     const response  = await res.json();
         let mydata = response.data;
          if(mydata){
            data = mydata[0]
            console.log("$$$$$$$$$$$$$$$  DATA Maker APPVIEWER")
            console.log(data)
      }else{
        ...
        <div>
            <div className="cardMKT williFlowItem">    
                ...
                <div className="cardMKTitemHidden">
                 <Suspense fallback={<p>Cargando...</p>}><AppViewerClient data={data}/></Suspense>
                </div> 
            </div>
